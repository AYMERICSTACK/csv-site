...
export default function ImageLightboxGallery({
  items,
  cardClassName = "flex max-w-md flex-col gap-8",
    ...
    <>
      <div className={cardClassName}>
        {validItems.map((item, index) => (
          <button
            ...
            className="group relative overflow-hidden rounded-[1rem] border border-orange-100 bg-white text-left shadow-sm transition hover:border-orange-200 hover:shadow-md"
            aria-label={`Agrandir ${item.title}`}
          >
            <div className="relative border-b border-orange-100 bg-neutral-100">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={item.imageUrl}
                alt={item.title}
                className="h-auto w-full object-contain bg-neutral-100 transition duration-300 group-hover:scale-[1.01]"
              />
            ...
            <div className="absolute inset-0 ring-0 transition group-hover:ring-1 group-hover:ring-orange-200" />
          </button>
        ))}
      </div>
      ...
      {activeItem ? (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-neutral-950/92 p-3 sm:p-6">
          ...
          <div className="pointer-events-none absolute left-4 top-4 z-[301] rounded-full border border-white/10 bg-white/10 px-3 py-1 text-xs font-semibold text-white/90 backdrop-blur">
