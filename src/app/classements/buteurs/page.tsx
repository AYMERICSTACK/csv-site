...
export default async function ClassementButeursPage({
  searchParams,
}: PageProps) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-neutral-500">
                  Classement complet des buteurs du club
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {categories.map((category) => {
                const href =
                  category === "Toutes"
                    ? "/classements/buteurs"
                    : `/classements/buteurs?categorie=${encodeURIComponent(
                        category,
                      )}`;
          ...
          <div className="mt-8 space-y-3">
              ...
              >
                <div className="flex items-center gap-4">
                  {player.photoConsent && player.photoUrl ? (
                    ...
                    <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-neutral-950 text-sm font-black text-white">
                      #{index + 1}
                    </div>
                  )}
            ...
            {!scorers.length && (
              <div className="rounded-2xl border border-dashed border-neutral-200 p-8 text-center text-sm font-semibold text-neutral-500">
                Aucun buteur enregistré dans cette catégorie.
              </div>
            )}
          </div>
        </div>
